...
if check_password():
    ...
    with st.sidebar:
        ...
        if st.button("Cerrar Sesión"):
            st.session_state["password_correct"] = False
            st.rerun()
        # Limpiar caché cuando el médico actualice la BD de fármacos
        if st.button("🔄 Recargar Base de Datos"):
            st.cache_data.clear()
            st.success("Caché limpiado. BD recargada.")
    ...
    if archivo_subido:
        ...
        variantes = df[df['ID'].str.startswith('rs', na=False)].head(20).copy()
